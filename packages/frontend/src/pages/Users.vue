  ...
  <q-drawer v-model="store.drawer" :class="store.darkMode ? `bg-dark` : `bg-light`">
    <q-scroll-area class="fit">
      ...
      <div class="text-center align-center">
        <q-btn v-if="hasMore && usersFound.length"
          class="q-mt-md"
          color="primary"
...
async function fetchUsers(reset = false) {
  ...
  const result = await trpc.users.search.query({
    term: userSearched.value.trim().replace(/\s+/g, ' '),
    page: page.value,
    pageSize
  })
  hasMore.value = result.length === pageSize
  usersFound.value = reset ? result : [...usersFound.value, ...result]
}
...
onMounted(async () => {
  ...
  if (userSearched.value.length >= 3 || userSearched.value === '*') {
    await fetchUsers(true)
  }
})
